...
export class NumberOfOrdersComponent implements OnInit {
  ...
  constructor(private http: HttpClient, @Inject('BASE_URL') private baseUrl: string) { 
    this.http.get<ClientsID[]>(baseUrl + 'ordersbyclient').subscribe(result => {
      this.clients = result;
  ...
  getTotal(event){
    this.http.post(this.baseUrl + 'ordersbyclient', {dateS: this.pickerS, dateE: this.pickerE, idClient : this.idClient, totalOrders: this.totalOrders}).subscribe(
      res=> {
        console.log(res);
