...
CREATE TABLE ventas.tiendaNewYork (
...
CREATE TABLE ventas.tiendaCalifornia (
	idTienda INT PRIMARY KEY,
	nomTienda VARCHAR (255) NOT NULL,
	telefono VARCHAR (25),
	email VARCHAR (255),
...
CREATE TABLE ventas.empleadosNewYork (
	...
	idJefe INT,
	FOREIGN KEY (idTienda) REFERENCES ventas.tiendaNewYork (idTienda) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE ventas.empleadosCalifornia (
	idEmpleado INT PRIMARY KEY,
	nombre VARCHAR (50) NOT NULL,
	apellido VARCHAR (50) NOT NULL,
	email VARCHAR (255) NOT NULL UNIQUE,
	...
	idTienda INT NOT NULL,
	idJefe INT,
	FOREIGN KEY (idTienda) REFERENCES ventas.tiendaCalifornia (idTienda) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE ventas.ordenesNewYork (
	...
	FOREIGN KEY (idTienda) REFERENCES ventas.tiendaNewYork (idTienda) ON DELETE CASCADE ON UPDATE CASCADE,
	FOREIGN KEY (idEmpleado) REFERENCES ventas.empleadosNewYork (idEmpleado) ON DELETE NO ACTION ON UPDATE NO ACTION
);

CREATE TABLE ventas.ordenesCalifornia (
	idOrden INT PRIMARY KEY,
	idCliente INT,
	estadoOrden tinyint NOT NULL,
	-- Order status: 1 = Pending; 2 = Processing; 3 = Rejected; 4 = Completed
	...
	FOREIGN KEY (idCliente) REFERENCES ventas.clientes (idCliente) ON DELETE CASCADE ON UPDATE CASCADE,
	FOREIGN KEY (idTienda) REFERENCES ventas.tiendaCalifornia (idTienda) ON DELETE CASCADE ON UPDATE CASCADE,
	FOREIGN KEY (idEmpleado) REFERENCES ventas.empleadosCalifornia (idEmpleado) ON DELETE NO ACTION ON UPDATE NO ACTION
);
...
CREATE TABLE produccion.inventarioCalifornia (
	idTienda INT,
	idProducto INT,
	cantidad INT,
	PRIMARY KEY (idTienda, idProducto),
